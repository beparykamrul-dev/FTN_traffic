...
func (f FRRRuntime) Apply(ctx context.Context, commands []string) error { if !f.Authorized{return ErrUnauthorized}; if !f.Approved{return ErrApprovalRequired}; if f.Runner==nil{return ErrBackendUnavailable}; for _,cmd:=range commands { if cmd=="" {return fmt.Errorf("empty FRR command")}; if _,err:=f.Runner.Run(ctx,"vtysh","-c",cmd); err!=nil{return err} }; return nil }
