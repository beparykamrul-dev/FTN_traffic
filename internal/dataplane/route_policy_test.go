...
func TestRoutePolicyRejectsInvalidCommunity(t *testing.T) {
...
func TestRoutePolicyAcceptsIPv6AndLargeCommunity(t *testing.T) {
	r := RouteBatch{RPKIValid:true, Routes:[]RouteIntent{{Prefix:"2001:db8:1::/48",Family:IPv6,NextHop:"2001:db8::1",Authorized:true,Community:[]string{"64500:1:100"}}}}
	if err := (RoutePolicy{RequireRPKI:true,MaxPrefixes:10}).Validate(r); err != nil { t.Fatalf("unexpected validation error: %v", err) }
}
