...
func (c *Controller) Run(ctx context.Context,r Request) error {
	...
	if err:=c.exec.Shift(ctx,r,r.Strategy); err!=nil { return c.rollback(ctx,r,err) }
	c.mu.Lock(); c.set(StateObserve); c.mu.Unlock()
	if err:=c.health.Healthy(ctx,r); err!=nil { return c.rollback(ctx,r,fmt.Errorf("post-shift health failed: %w",err)) }
	c.mu.Lock(); c.set(StateCommit); c.mu.Unlock()
	if err:=c.exec.Commit(ctx,r); err!=nil { return c.rollback(ctx,r,err) }
	c.record(ctx,"migration_committed",r,""); return nil
}
func (c *Controller) rollback(ctx context.Context,r Request,cause error) error { c.mu.Lock(); c.set(StateRollback); c.mu.Unlock(); c.record(ctx,"migration_rollback",r,cause.Error()); if err:=c.exec.Rollback(ctx,r); err!=nil { c.mu.Lock(); c.set(StateFailed); c.mu.Unlock(); return fmt.Errorf("%v; rollback failed: %w",cause,err) }; return fmt.Errorf("migration rolled back: %w",cause) }
