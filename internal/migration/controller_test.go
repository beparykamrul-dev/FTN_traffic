...
func TestApprovalFailureDoesNotConsumeKey(t *testing.T){e:=&fakeExec{};c:=NewController(e,fakeApproval{false},fakeHealth{true});r:=req("approval-retry");if err:=c.Run(nil,r);err==nil{t.Fatal("expected denial")};c.approval=fakeApproval{true};if err:=c.Run(nil,r);err!=nil{t.Fatalf("retry after approval failed: %v",err)}}
func TestPreflightFailureDoesNotConsumeKey(t *testing.T){e:=&fakeExec{failPreflight:true};c:=NewController(e,fakeApproval{true},fakeHealth{true});r:=req("preflight-retry");if err:=c.Run(nil,r);err==nil{t.Fatal("expected preflight failure")};e.failPreflight=false;if err:=c.Run(nil,r);err!=nil{t.Fatalf("retry after preflight failed: %v",err)}}
